...
- **账号中心**：可从管理鉴权保护的 `/v0/management/account-pool` 接口自动加载服务器私有快照，并显示“全部 / 未导入 / 已导入”数量和筛选。也可用一份或多份本地 `cami-list.html`、文本或 JSON 清单临时替换服务器快照，本地导入默认标记为“未导入”。支持搜索、敏感字段显隐、复制、移除和 TOTP 验证码；服务器凭据不会嵌入前端构建或浏览器存储，本页也不会执行 CLIProxy 或 Sub2API 导入。
