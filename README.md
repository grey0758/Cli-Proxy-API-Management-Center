...
- **Pending Account Pool**: automatically load an optional private snapshot from the management-authenticated `/v0/management/account-pool` endpoint, or temporarily replace it with one or more local `cami-list.html`, text, or JSON lists. Rows support search, mask/reveal, copy, remove, and TOTP codes. Server credentials are never embedded in the frontend build or browser storage, and this page does not perform a CLIProxy or Sub2API import.
