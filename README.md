...
- **Account Center**: automatically load an optional private snapshot from the management-authenticated `/v0/management/account-pool` endpoint, with `All`, `Pending`, and `Imported` counts and filters. One or more local `cami-list.html`, text, or JSON lists can temporarily replace the server snapshot and default to `Pending`. Rows support search, mask/reveal, copy, remove, and TOTP codes. Server credentials are never embedded in the frontend build or browser storage, and this page does not perform a CLIProxy or Sub2API import.
